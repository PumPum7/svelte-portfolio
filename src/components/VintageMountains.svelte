<script lang="ts">
	...
	const backMountainStyle = $derived(
		`transform: translateY(${scrollY * 0.15}px) scale(1.05); filter: sepia(1) contrast(0.8) brightness(1.1);`
	);
</script>
...
<div class="pointer-events-none absolute inset-0 z-0 overflow-hidden select-none">
	...
	<div
		class="absolute right-0 bottom-0 left-0 h-3/4 origin-bottom transition-transform duration-75 ease-out"
		style={backMountainStyle}
	>
		<svg viewBox="0 0 1440 400" class="absolute bottom-0 h-full w-full" preserveAspectRatio="none">
			<path
	...
	<div
		class="absolute right-0 bottom-0 left-0 h-2/3 origin-bottom"
